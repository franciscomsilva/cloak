...
std::unique_ptr<ModuleRtpRtcpImpl2> CreateRtpRtcpModule(
    Clock *clock, ReceiveStatistics *receive_statistics,
    Transport *outgoing_transport, RtcpRttStats *rtt_stats,
    RtcpPacketTypeCounterObserver *rtcp_packet_type_counter_observer,
    RtcpCnameCallback *rtcp_cname_callback, bool non_sender_rtt_measurement,
    uint32_t local_ssrc) {
...
std::unique_ptr<NackRequester>
MaybeConstructNackModule(TaskQueueBase *current_queue,
                         NackPeriodicProcessor *nack_periodic_processor,
                         const VideoReceiveStream::Config &config, Clock *clock,
...
RtpVideoStreamReceiver2::RtcpFeedbackBuffer::RtcpFeedbackBuffer(
    KeyFrameRequestSender *key_frame_request_sender, NackSender *nack_sender,
...
void RtpVideoStreamReceiver2::RtcpFeedbackBuffer::SendNack(
    const std::vector<uint16_t> &sequence_numbers, bool buffering_allowed) {
  RTC_DCHECK_RUN_ON(&packet_sequence_checker_);
...
void RtpVideoStreamReceiver2::RtcpFeedbackBuffer::SendLossNotification(
    uint16_t last_decoded_seq_num, uint16_t last_received_seq_num,
...
RtpVideoStreamReceiver2::RtpVideoStreamReceiver2(
    TaskQueueBase *current_queue, Clock *clock, Transport *transport,
    RtcpRttStats *rtt_stats, PacketRouter *packet_router,
    const VideoReceiveStream::Config *config,
    ReceiveStatistics *rtp_receive_statistics,
    RtcpPacketTypeCounterObserver *rtcp_packet_type_counter_observer,
    RtcpCnameCallback *rtcp_cname_callback,
    NackPeriodicProcessor *nack_periodic_processor, NackSender *nack_sender,
    KeyFrameRequestSender *keyframe_request_sender,
    OnCompleteFrameCallback *complete_frame_callback,
    rtc::scoped_refptr<FrameDecryptorInterface> frame_decryptor,
    rtc::scoped_refptr<FrameTransformerInterface> frame_transformer)
    : clock_(clock), config_(*config), packet_router_(packet_router),
      ntp_estimator_(clock), rtp_header_extensions_(config_.rtp.extensions),
      ...
      receiving_(false), last_packet_log_ms_(-1),
      rtp_rtcp_(CreateRtpRtcpModule(
          clock, rtp_receive_statistics_, transport, rtt_stats,
          rtcp_packet_type_counter_observer, rtcp_cname_callback,
...
RtpVideoStreamReceiver2::~RtpVideoStreamReceiver2() {
  ...
  //printResolutionDebugInfo();

	//
}

void RtpVideoStreamReceiver2::AddReceiveCodec(
    uint8_t payload_type, VideoCodecType video_codec,
    const std::map<std::string, std::string> &codec_params, bool raw_payload) {
  RTC_DCHECK_RUN_ON(&packet_sequence_checker_);
...
RtpVideoStreamReceiver2::ParseGenericDependenciesExtension(
    const RtpPacketReceived &rtp_packet, RTPVideoHeader *video_header) {
...
void RtpVideoStreamReceiver2::OnReceivedPayloadData(
    rtc::CopyOnWriteBuffer codec_payload, const RtpPacketReceived &rtp_packet,
...
void RtpVideoStreamReceiver2::OnAssembledFrame(
  ...
  RTC_DCHECK(frame);
#if DEBUG_PRINT_FRAMES_HOOK
  const EncodedImage &encoded_image = frame->EncodedImage();
  const std::string resolution_key =
      std::to_string(encoded_image._encodedWidth) + "X" +
      std::to_string(encoded_image._encodedHeight);

  // Check if resolution already exists and update counter. Otherwise create
  // entry for it
  if (resolution_counter.find(resolution_key) != resolution_counter.end()) {
    const int counter = resolution_counter.at(resolution_key);
    resolution_counter.insert_or_assign(resolution_key, counter + 1);
  } else {

    resolution_counter.insert(std::pair(resolution_key, 1));
  }

  		
  	// Track the resolution changes
	const std::string resolution_tracker_key =
        last_resolution + "-" + resolution_key;

	ResolutionChange res_change;
	

  if (!last_resolution.empty() && last_resolution.compare(resolution_key) != 0) {

    // If an entry for this resolution change already exists; update counter
    if (resolution_change_tracker.find(resolution_tracker_key) !=
        resolution_change_tracker.end()) {
			res_change = resolution_change_tracker.at(resolution_tracker_key);
			res_change.counter += 1;

			resolution_change_tracker.insert_or_assign(resolution_tracker_key,res_change);


    }
	// Otherwise create entry and set counter to 1
    else if(!last_resolution.empty()){
      res_change.origin = last_resolution;
      res_change.destination = resolution_key;
      res_change.counter = 1;

      resolution_change_tracker.insert(std::pair(resolution_tracker_key, res_change));
  ...
  // SSRC
  uint32_t ssrc;
  for(auto packet_info : frame->PacketInfos()){
    ssrc = packet_info.ssrc();
     // Check if ssrc already exists and update counter. Otherwise create
    // entry for it
    if (ssrc_counter.find(ssrc) != ssrc_counter.end()) {
      const int counter = ssrc_counter.at(ssrc);
      ssrc_counter.insert_or_assign(ssrc, counter + 1);
    } else {
      ssrc_counter.insert(std::pair(ssrc, 1));
    }

        
      // Track the ssrc changes
    SsrcChange ssrc_change;
    

    if (last_ssrc != 0 && last_ssrc != ssrc) {

      // If an entry for this resolution change already exists; update counter
      if (ssrc_change_tracker.find(ssrc) !=
          ssrc_change_tracker.end()) {
        ssrc_change = ssrc_change_tracker.at(ssrc);
        ssrc_change.counter += 1;

        ssrc_change_tracker.insert_or_assign(ssrc,ssrc_change);


      }
    // Otherwise create entry and set counter to 1
      else if(last_ssrc != 0){
        ssrc_change.origin = last_ssrc;
        ssrc_change.destination = ssrc;
        ssrc_change.counter = 1;

        ssrc_change_tracker.insert(std::pair(ssrc, ssrc_change));
  ...
  probePrint("rtp_video_stream_receiver2::OnAssembledFrame");
...
RtpVideoStreamReceiver2::LastReceivedKeyframePacketMs() const {
  RTC_DCHECK_RUN_ON(&packet_sequence_checker_);
...
void RtpVideoStreamReceiver2::UpdatePacketReceiveTimestamps(
    const RtpPacketReceived &packet, bool is_keyframe) {
