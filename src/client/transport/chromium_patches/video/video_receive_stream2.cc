...
#include "hooks/recorded_frames.h"
...
public:
  explicit WebRtcRecordableEncodedFrame(
      ...
      : buffer_(frame.GetEncodedData()), render_time_ms_(frame.RenderTime()),
...
VideoReceiveStream2::VideoReceiveStream2(
    TaskQueueFactory *task_queue_factory, Call *call, int num_cpu_cores,
    PacketRouter *packet_router, VideoReceiveStream::Config config,
    CallStats *call_stats, Clock *clock, VCMTiming *timing,
    NackPeriodicProcessor *nack_periodic_processor,
    DecodeSynchronizer *decode_sync)
    : task_queue_factory_(task_queue_factory),
      transport_adapter_(config.rtcp_send_transport),
      config_(std::move(config)), num_cpu_cores_(num_cpu_cores), call_(call),
      clock_(clock), call_stats_(call_stats), source_tracker_(clock_),
      stats_proxy_(config_.rtp.remote_ssrc, clock_, call->worker_thread()),
      rtp_receive_statistics_(ReceiveStatistics::Create(clock_)),
      timing_(timing), video_receiver_(clock_, timing_.get()),
      rtp_video_stream_receiver_(
          call->worker_thread(), clock_, &transport_adapter_,
          call_stats->AsRtcpRttStats(), packet_router, &config_,
          rtp_receive_statistics_.get(), &stats_proxy_, &stats_proxy_,
      ...
      decode_queue_(task_queue_factory_->CreateTaskQueue(
          "DecodingQueue", TaskQueueFactory::Priority::HIGH)) {
...
void VideoReceiveStream2::OnCompleteFrame(std::unique_ptr<EncodedFrame> frame) {
  ...
  #if MEASURE_DECODING_TIME_HOOKS
    auto time_before_decoding= std::chrono::high_resolution_clock::now();
  #endif


  #if DECODE_ON_COMPLETE_FRAME_HOOK

    const EncodedImage& encoded_image = frame->EncodedImage();
    int replaced = 0;

    /********************************* START OF FRAME PARSING ******************************/
   int first_part_offset = 3;

   if(encoded_image._frameType == VideoFrameType::kVideoFrameKey){ //Key Frame
        first_part_offset = 10;
   }

/********************************* END OF FRAME PARSING ********************************/

    retrieveDataFromFrame(encoded_image.mutable_data(), encoded_image.size(), first_part_offset,(int)encoded_image._frameType);
  #endif

  /*****************************************************************************/
  /*****************************************************************************/

   #if  RECORD_FRAME_HOOK_RECEIVER
        #if !DECODE_ON_COMPLETE_FRAME_HOOK
            const EncodedImage& encoded_image = frame->EncodedImage();
        #endif
  ...
  #if REPLACE_FRAME_AT_DECODING_HOOK
    #if !(DECODE_ON_COMPLETE_FRAME_HOOK)
      const EncodedImage& encoded_image = frame->EncodedImage();
    #endif
     ...
     if((int)encoded_image._frameType == 3){ //Key
      ...
      if(lastKeyFrameWidth == 640 && lastKeyFrameHeight == 360){
        if(encoded_image.size() > recorded_key_frame_length_640x360_small){
      ...
      if(lastKeyFrameWidth == 640 && lastKeyFrameHeight == 480){
        if(encoded_image.size() > recorded_key_frame_length_640x480_small){
      ...
      if(lastKeyFrameWidth == 960 && lastKeyFrameHeight == 540){
        if(encoded_image.size() > recorded_key_frame_length_960x540_small){
      ...
      if(lastKeyFrameWidth == 1280 && lastKeyFrameHeight == 720){
        if(encoded_image.size() > recorded_key_frame_length_1280x720_small){
      ...
      if(lastKeyFrameWidth == 640 && lastKeyFrameHeight == 360){
        if(encoded_image.size() > recorded_delta_frame_length_640x360_small){
      ...
      if(lastKeyFrameWidth == 640 && lastKeyFrameHeight == 480){
        if(encoded_image.size() > recorded_delta_frame_length_640x480_small){
      ...
      if(lastKeyFrameWidth == 960 && lastKeyFrameHeight == 540){
        if(encoded_image.size() > recorded_delta_frame_length_960x540_small){
      ...
      if(lastKeyFrameWidth == 1280 && lastKeyFrameHeight == 720){
        if(encoded_image.size() > recorded_delta_frame_length_1280x720_small){
       ...
       if(lastKeyFrameWidth == 0 && lastKeyFrameHeight == 0){
        if(encoded_image.size() > recorded_delta_frame_length_1280x720_small){
          frame->SetMutableBuffer(recorded_delta_frame_1280x720_small, recorded_delta_frame_length_1280x720_small);
          frame->SetLength(recorded_delta_frame_length_1280x720_small);
          frame->SetSize(recorded_delta_frame_length_1280x720_small);
    ...
    if(replaced == 0){
     probePrint("[video_receive_stream.cc] Frame not replaced Width: " + std::to_string(lastKeyFrameWidth) + " Height: " + std::to_string(lastKeyFrameHeight) + " Codec: " +  std::to_string(frame->GetCodec()));
    /*printEncodedImageInfo(lastKeyFrameWidth,
                lastKeyFrameHeight,
                encoded_image.size(),
                encoded_image.size(),
                encoded_image._completeFrame,
    ...
    #if READ_AT_COMPLETE_FRAME_HOOK
      readFromPipe();
    #endif

    #if MEASURE_DECODING_TIME_HOOKS
        auto time_after_delivery = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double, std::milli> duration = time_after_delivery - time_before_decoding;
        printf("[video_receive_stream2.cc] Time to decode frame: %f\n", duration.count());
    #endif
...
void VideoReceiveStream2::OnDecodableFrameTimeout(TimeDelta wait_time) {
  if (!call_->worker_thread()->IsCurrent()) {
    call_->worker_thread()->PostTask(
        ToQueuedTask(task_safety_, [this, wait_time] {
          OnDecodableFrameTimeout(wait_time);
        }));
    return;
  }
...
void VideoReceiveStream2::HandleKeyFrameGeneration(
    bool received_frame_is_keyframe, int64_t now_ms,
    bool always_request_key_frame, bool keyframe_request_is_due) {
