...
std::unique_ptr<EncodedFrame> FrameBuffer::GetNextFrame() {
  ...
  for (FrameMap::iterator& frame_it : frames_to_decode_) {
    ...
    global_dropped_frames_by_decoder+=1;
    probePrint("[global_dropped_frames_by_decoder]: " + std::to_string(global_dropped_frames_by_decoder));
#endif
...
int64_t FrameBuffer::InsertFrame(std::unique_ptr<EncodedFrame> frame) {
  ...
  RTC_DCHECK(frame);
#if DEBUG_PRINT_FRAMES_HOOK
    global_frame_receive_counter+=1;
    probePrint("[global_frame_receive_counter]: " + std::to_string(global_frame_receive_counter));
#endif
  MutexLock lock(&mutex_);
