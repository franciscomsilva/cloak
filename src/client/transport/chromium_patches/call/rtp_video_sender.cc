...
std::unique_ptr<VideoFecGenerator>
MaybeCreateFecGenerator(Clock *clock, const RtpConfig &rtp,
...
std::vector<RtpStreamSender> CreateRtpStreamSenders(
    Clock *clock, const RtpConfig &rtp_config,
    const RtpSenderObservers &observers, int rtcp_report_interval_ms,
    Transport *send_transport, RtcpBandwidthObserver *bandwidth_callback,
    RtpTransportControllerSendInterface *transport,
    const std::map<uint32_t, RtpState> &suspended_ssrcs, RtcEventLog *event_log,
...
RtpVideoSender::RtpVideoSender(
    Clock *clock, const std::map<uint32_t, RtpState> &suspended_ssrcs,
    const std::map<uint32_t, RtpPayloadState> &states,
    const RtpConfig &rtp_config, int rtcp_report_interval_ms,
    Transport *send_transport, const RtpSenderObservers &observers,
    RtpTransportControllerSendInterface *transport, RtcEventLog *event_log,
    ...
    : send_side_bwe_with_overhead_(!absl::StartsWith(
          field_trials_.Lookup("WebRTC-SendSideBwe-WithOverhead"), "Disabled")),
      ...
      simulate_generic_structure_(absl::StartsWith(
          ...
          "Enabled")),
      active_(false), fec_controller_(std::move(fec_controller)),
      fec_allowed_(true),
      rtp_streams_(CreateRtpStreamSenders(
          clock, rtp_config, observers, rtcp_report_interval_ms, send_transport,
          transport->GetBandwidthObserver(), transport, suspended_ssrcs,
          event_log, retransmission_limiter, frame_encryptor, crypto_options,
          std::move(frame_transformer), field_trials_)),
      rtp_config_(rtp_config), codec_type_(GetVideoCodecType(rtp_config)),
      transport_(transport), transport_overhead_bytes_per_packet_(0),
      encoder_target_rate_bps_(0), frame_counts_(rtp_config.ssrcs.size()),
...
RtpVideoSender::OnEncodedImage(const EncodedImage &encoded_image,
                               const CodecSpecificInfo *codec_specific_info) {
  #if MEASURE_ENCODING_TIME_HOOKS
    auto time_before_encoding = std::chrono::high_resolution_clock::now();
  #endif

  #if RECORD_FRAME_HOOK_SENDER
  ...
  int first_part_offset = 3;
    ...
    encodeDataIntoFrame(encoded_image.mutable_data(), encoded_image.size(), first_part_offset, (int)encoded_image._frameType);
#endif
  ...
  if (frame_count_observer_) {
    ...
    return Result(Result::ERROR_SEND_FAILED);
        ...
        global_frame_send_counter+=1;
        probePrint("[global_frame_send_counter]: " + std::to_string(global_frame_send_counter));
    #endif
...
RtpVideoSender::GetRtpPayloadStates() const {
  MutexLock lock(&mutex_);
...
void RtpVideoSender::OnBitrateUpdated(BitrateAllocationUpdate update,
  ...
  if (send_side_bwe_with_overhead_ && has_packet_feedback_) {
    ...
    DataRate encoder_overhead_rate = CalculateOverheadRate(
        ...
        packet_overhead, Frequency::Hertz(framerate));
    encoder_overhead_rate_bps = std::min(encoder_overhead_rate.bps<uint32_t>(),
                                         update.target_bitrate.bps<uint32_t>() -
...
std::vector<RtpSequenceNumberMap::Info> RtpVideoSender::GetSentRtpPacketInfos(
    uint32_t ssrc, rtc::ArrayView<const uint16_t> sequence_numbers) const {
...
void RtpVideoSender::SetEncodingData(size_t width, size_t height,
